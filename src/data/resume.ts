...
export type TraitItem = { label: string; text: string };

export type AboutMeSection =
  | { id: 'intro'; title: string; paragraphs: string[] }
  | {
      id: 'strengths';
      title: string;
      strengths: TraitItem[];
...
export const aboutMeSections: AboutMeSection[] = [
  ...
  },
  {
    ...
    strengths: [
      {
        label: '협업과 커뮤니케이션',
        text: '저는 다양한 직군과 원활하게 소통하며 협업하는 것을 중요하게 생각합니다. AI 서비스를 개발하면서 기획, 운영, 개발 담당자와 지속적으로 소통하며 요구사항을 조율하고 이를 모델과 시스템 설계에 반영했습니다. 주변에서도 "의사소통이 부드럽다"는 이야기를 자주 들으며, 여러 ML 서비스 개발 과정에서 요구사항을 신속하게 반영해 서비스 품질과 운영 안정성을 높이는 데 기여했습니다.',
      },
      {
        label: '빠른 학습과 적용',
        text: '저는 새로운 기술을 빠르게 습득해 실제 서비스에 적용하는 것을 즐깁니다. 강화학습, LangGraph, MLflow, Airflow 등 처음 접하는 기술도 논문과 기술 문서를 학습해 프로젝트에 직접 적용했습니다. DQN 기반 HVAC 제어 프로젝트에서는 초기 성능이 기대에 미치지 않았지만 보상 함수를 지속적으로 개선하고 반복 실험을 수행한 끝에 10개 이상의 유닛을 안정적으로 제어하는 시스템을 구축했으며, 실외기 전력 사용량을 14.9% 절감하는 성과를 달성했습니다.',
      },
    ],
    weaknesses: [
      {
        label: '완성도에 대한 집착',
        text: '저의 단점은 결과물의 완성도를 높이기 위해 세부적인 부분까지 오래 고민하는 성향입니다. 재실 예측 서비스를 개발할 당시 기능적으로는 문제가 없는 코드였지만 구조를 더 개선하기 위해 리팩토링에 예상보다 많은 시간을 투자한 경험이 있습니다. 이를 통해 처음부터 완벽한 결과물을 만드는 것보다 빠르게 검증하고 지속적으로 개선하는 것이 더 중요하다는 점을 배웠고, 현재는 우선 운영 가능한 서비스를 구축한 뒤 운영 데이터를 기반으로 성능과 품질을 지속적으로 개선하는 방식으로 업무를 수행하고 있습니다.',
      },
      {
        label: '신중한 의사결정',
        text: '또한 여러 선택지가 있을 때 충분히 비교·검증하려는 성향으로 인해 의사결정에 시간이 다소 걸린다는 단점도 있습니다. 에너지 사용량 예측 서비스를 개발할 당시 여러 모델을 비교하며 성능뿐 아니라 학습 안정성, 추론 속도, 운영 비용까지 함께 검토하다 보니 초기 의사결정에 예상보다 많은 시간을 사용한 경험이 있습니다. 이후에는 서비스 목적과 운영 환경을 먼저 고려해 후보를 빠르게 좁힌 뒤 검증하는 방식으로 업무를 개선하며 개발 속도와 서비스 품질을 함께 높이고 있습니다.',
      },
    ],
  },
];
